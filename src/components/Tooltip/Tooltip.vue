<template>
  <div class="dyd-tooltip" v-on="outerEvents">
    <!-- 触发的元素，设置为一个插槽 -->
    <div class="dyd-tooltip__trigger" ref="triggerRef" v-on="events">
      <slot />
    </div>
    <!-- 要显示的提示内容 -->
    <!-- 考虑有些时候提示内容会比较复杂，因此这里仍然是提供插槽的形式 -->
    <div class="dyd-tooltip__popper" ref="popperRef" v-if="isOpen">
      <slot name="content">
        {{ content }}
      </slot>
      <!-- 箭头 -->
